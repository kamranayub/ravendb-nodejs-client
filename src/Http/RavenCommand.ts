...
export abstract class RavenCommand<TResult> {
  ...
  ): RavenCommandResponsePipeline<T> {
  ...
  public async send(
    agent: any,
    requestOptions: HttpRequestParameters
  ): Promise<{ response: HttpResponse; bodyStream: stream.Readable }> {
    const { body, uri, fetcher, ...restOptions } = requestOptions;
    log.info(
      `Send command ${this.constructor.name} to ${uri}${
        body ? " with body " + body : ""
      }.`
    );
  ...
  public async processResponse(
    ...
    url: string
  ): Promise<ResponseDisposeHandling> {
    if (!response) {
      return "Automatic";
    }
    ...
    try {
      ...
      log.error(
        err,
        `Error processing command ${this.constructor.name} response.`
      );
      throwError(
        "RavenException",
        `Error processing command ${this.constructor.name} response: ${err.stack}`,
        err
      );
    } finally {
      closeHttpResponse(response);
  ...
  protected _addChangeVectorIfNotNull(
    changeVector: string,
  ...
  protected _reviveResultTypes<TResponse extends object>(
    ...
    return conventions.objectMapper.fromObjectLiteral<TResponse>(
      raw,
      typeInfo,
      knownTypes
    );
  ...
  protected async _parseResponseDefaultAsync(
    bodyStream: stream.Stream
  ): Promise<string> {
    let body: string = null;
    this.result = await this._defaultPipeline((_) => (body = _)).process(
      bodyStream
    );
    return body;
  }
  ...
  protected static _throwInvalidResponse(cause: Error): void {
    throwError(
      ...
      "Response is invalid: " + cause.message,
      cause
    );
  }
